...
import net.merise.platform.dao.estate.PayCostDao;
import net.merise.platform.dao.estate.PayCostItemDao;
...
public class PayCostServiceImpl implements PayCostService {
	...
	public Map<String, Object> payList(Map<String,Object> params) {
		...
		if(list.size()>0){
			...
			double moneny = 0;
			if(owner != null){
				map.put("owner_name", list.get(0).get("owner_name"));//业主姓名
			...
			map.put("owner_name", "");//业主姓名
			map.put("money", "");//业主余额
			map.put("rows", new ArrayList<Map<String, Object>>());//结果集
			map.put("total", 0);//结果集
			map.put("countMoneny", "");//缴费金额
			map.put("q_money", "");//欠费金额
		}
		return map;
	}

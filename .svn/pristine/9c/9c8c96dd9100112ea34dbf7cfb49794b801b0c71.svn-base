...
public class EstateHouseDetailController {
	...
	/**
	 * 初始化房屋
	 * @param building_idInit 楼盘编号
	 * @param dong_name 栋号
	 * @param start_floor 开始楼层
	 * @param finish_floor 结束楼层
	 * @param unit 单元号
	 * @param house_sort 物业类型
	 * @param floor_count 每层户数
	 * @param house_type 房屋户型
	 * @param house_build_area 建筑面积数组
	 * @param house_in_area 套内面积数组
	 */
	@RequestMapping("/initHouse")
	@ResponseBody
	public JSONObject initHouse(String building_idInit,String dong_name,String start_floor, String finish_floor, String unit,String house_sort,String floor_count,String[] house_type,String[] house_build_area,String[] house_in_area){
		JSONObject result = new JSONObject();
		start_floor = Coder.NullToBlank(start_floor);
		finish_floor = Coder.NullToBlank(finish_floor);
		floor_count = Coder.NullToBlank(floor_count);
		try {
			if ("".equals(start_floor) || "".equals(finish_floor)) {
				result.put("status", false);
				result.put("message", "起始层数或结束层数不能为空!");
			} else {
				// 查询楼盘已创建的的楼盘数据
				List<EstateHouseDetail> estateHouseDetails = estateHouseDetailService.findEstateHouseDetailByBuildingId(building_idInit);
				// 查询房屋状态编号
...
//				estateHouseDetailService.deleteByBuildingAndDongAndUnits(map);
				// 循环创建房屋
				for (int i = Integer.parseInt(start_floor); i <= Integer.parseInt(finish_floor); i++) {//循环楼层
					for (int j = 1; j <= Integer.parseInt(floor_count); j++) {//循环每层户数
						...
						// 如果房屋存在修改数据, 如果不存在新增数据
						JSONObject object = this.isHouseExist(estateHouseDetails, estateHouseDetail);
						System.out.println(estateHouseDetail.getHouse_location()+", "+object.getBooleanValue("status")+", "+object.getString("house_id"));
						if (object.getBooleanValue("status")) {
							estateHouseDetail.setHouse_id(object.getString("house_id"));
							estateHouseDetailService.update(estateHouseDetail);
						} else {
	 ...
	 * 判断房屋是否存在
	 * @param list 房屋集合
	 * @param detail 需要匹配的房屋对象
	 * @return true/false
	 */
	private JSONObject isHouseExist(List<EstateHouseDetail> list, EstateHouseDetail detail) {
		JSONObject object = new JSONObject();
		object.put("status", false);
		object.put("house_id", "");
		for (EstateHouseDetail estateHouseDetail : list) {
			boolean dongStatus = estateHouseDetail.getHouse_dong().equals(detail.getHouse_dong());
			boolean unitStatus = estateHouseDetail.getHouse_units().equals(detail.getHouse_units());
			boolean locationStatus = estateHouseDetail.getHouse_location().equals(detail.getHouse_location());
			boolean ztStatus = estateHouseDetail.getHouse_status().equals(detail.getHouse_status());
			if (dongStatus && unitStatus && locationStatus && ztStatus) {
				object.put("status", true);
				object.put("house_id", estateHouseDetail.getHouse_id());
			}
		}
		return object;
	}
